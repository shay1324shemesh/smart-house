...
let deviceStatus;
let cnt=false;
export default function PowerToggle(props) {
    console.log(props.roomArr);

    
    const [checkedPower,setPowerChecked] = useState(cnt);

    let indexNum = props.indexNum
    console.log(indexNum);
    const handleChangePower = val => {
        if(checkedPower === false){
          deviceStatus = 'דוד דלוק'
          setPowerChecked(val)
        }
        else if(checkedPower === true){
          deviceStatus = ' דוד כבוי  '
          setPowerChecked(val)
        }
      }

      const showPrPower=()=>{
        if(deviceStatus === 'דוד דלוק'){
            cnt = true
        }
        else{
            cnt =false
      ...
      // var resetPowerDevice=()=>{
      //   props.roomArr[indexNum].roomDevicePower = ""
      // }
    
      // var newPowerDevice=()=>{
      //   props.roomArr[indexNum].roomDevicePower = "power"
      // }

  return (
    <div className='powerToggleMain'>
<div>
        <button className='removePower' onClick={props.changeOtherSidePower}> X הסר דוד </button>
    <p className='PowerPCss'>מצב דוד : {deviceStatus}</p><br />
    ...
    <p className='svgPower'><svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" fill="firebrick" class="bi bi-thermometer-snow" viewBox="0 0 16 16">
